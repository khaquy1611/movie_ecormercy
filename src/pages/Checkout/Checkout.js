...
import { USER_LOGIN, TOKEN } from '../../ultils/settings/config';
import { useDispatch, useSelector } from 'react-redux';
import {
  BookingChairAction,
  getDetailsRoomTicketAction,
  BookingTicketAction,
...
import { getInfoUserAction } from '../../redux/actions/UserActions';
...
function Checkout() {
  ...
  useEffect(() => {
    ...
    const action = getDetailsRoomTicketAction(id);
  ...
  }, [dispatch, id, userLogin.taiKhoan]);
    ...
    return danhSachGhe.map((ghe, index) => {
      ...
      return (
        <Fragment key={index}>
          <button
            onClick={() => {
              const action = BookingChairAction(ghe, id);
              dispatch(action);
            }}
      ...
      <div className="grid grid-cols-12">
        ...
        <div className="col-span-3">
          ...
          >
            <button
              onClick={() => {
                const thongTinDatVe = new InfoBookTicket();
